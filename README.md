...
C++ smart contracts that implement the SikaChain protocol, deployed at chain genesis. Modeled on Vaulta's [system-contracts](https://github.com/VaultaFoundation/system-contracts) and forked to encode SikaChain's distinctive tokenomics and governance.

## What's in here

| Contract | Deployed to | Responsibility |
|---|---|---|
| `sika.system` | `sika` (privileged) | The core: BPs, voting, REX, RAM, halving inflation, vote-removal enforcement |
...
## What's different from Vaulta

The core EOSIO/Antelope DPoS pattern (registered BPs with weighted votes, REX, RAM Bancor, CPU/NET staking) is intentionally identical so that block explorers, wallets, and tools work unmodified. The SikaChain-unique pieces:
...
2. **Deploy `sika.token`** to `sika.token` and create the SIKA symbol with 8.64B max supply and `sika` as issuer.
3. **Deploy `sika.system`** to `sika` (privileged). Genesis block producer is `sikaio` (protocol account).
4. **Activate protocol features** required by the system contract.
5. **Call `sika::init(0, "4,SIKA")`** to seed the genesis timestamp and RAM market.
...
All ≤ 12 characters (AntelopeOS constraint). Defined in `sika.system.hpp` and mirrored across the codebase.

| Account | Length | Purpose |
|---|---|---|
| `sikaio` | 6 | Protocol privileged account (was `eosio`; genesis producer, native handlers) |
| `sika` | 4 | Privileged — `sika.system` contract host |
...
The same constants appear in **three places** and **must stay in sync**:

| Constant | TypeScript (`domain.ts`) | C++ (`sika.system.hpp`) | Prisma (`schema.prisma`) |
|---|---|---|---|
| 1M SIKA stake floor | `STAKE_FLOOR_RAW = 1_000_000n × 10_000n` | `bp_stake_floor = 10'000'000'000ll` | (referenced in BpComplianceState checks) |
| 95% uptime | `UPTIME_FLOOR = 95.0` | `uptime_floor_bps = 9500` | (column on Producer model) |
| 7-day SLA | `ISSUE_SLA_DAYS = 7` | `sla_seconds = 7*24*60*60` | (column on Producer.openIssueAt) |
| +5% Rep boost cap | `REP_BOOST_CAP = 5.0` | `rep_boost_cap_bps = 500` | (column on Representative.boostApy with CHECK constraint) |
| 30 BPs per vote | (validated in voteproducer schema) | `max_bps_per_vote = 30` | (Vote.bpSelection array max length) |
| 0.5% RAM fee | (not modeled off-chain) | `ram_fee_bps = 50` | — |
| Halving every 4 years | `halving_epoch_years = 4` in `inflationRateForYear` | `halving_epoch_years = 4` | — |
...
| 75/25 BP split | (informational on landing page) | `top21_share_bps = 7500`, `standby_share_bps = 2500` | — |

If you change a constant in one place, **change it in all three** and update the integration tests.

## Testing

Contract tests live in a separate repo: **https://github.com/rroland10/sika-chain-contract-tests** (Spring BOOST.Test, 45 cases). Clone next to this tree as `sika chain testing script` or `sika-chain-contract-tests`, or set `SIKA_TESTS_DIR`.

```bash
git clone https://github.com/rroland10/sika-chain-contract-tests.git ../sika-chain-contract-tests
export SPRING_BUILD_PATH=/path/to/spring/build   # Spring built with -DSIKACHAIN=ON
cd contracts && SIKACHAIN=1 ./build.sh --with-tests
# or from Spring: make sika_unit_tests && ./sikachain-tests/sika_unit_tests
```

The tests in `contracts/tests/` (built with `--with-tests`) use Spring's BOOST framework to exercise full transaction flows. They cover:
...
- **Auth wiring:** After elections seat new Guardians, `sika.guard::active` must be reconfigured to require 6-of-9 of the new seats. This is done via a Guardian-approved proposal that calls `sika::updateauth`.
